...
object DHUtil {
    ...
    // 键值对线程安全
    private val keyPairGeneratorThreadLocal = ThreadLocal<KeyPairGenerator>()

    init {
        Security.addProvider(BouncyCastleProvider())
        keyPairGeneratorThreadLocal.set(createKeyPairGenerator())
    }
    ...
    private fun createKeyPairGenerator(): KeyPairGenerator {
        return try {
            ...
            throw RuntimeException("Failed to initialize KeyPairGenerator", e)
        }
    }

    private fun getKeyPairGenerator(): KeyPairGenerator {
        return keyPairGeneratorThreadLocal.get()
    }
