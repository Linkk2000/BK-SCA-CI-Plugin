...
import com.tencent.bk.devops.plugin.pojo.kubernetes.CreateServiceReq
...
class KubernetesBuildApi : BaseApi() {
    ...
    fun createJob(dispatchJobReq: DispatchJobReq): Result<DispatchTaskResp?> {
        val path = "/dispatchkubernetes/api/build/job/create"
    ...
    fun createService(createServiceReq: CreateServiceReq): Result<DispatchTaskResp?> {
        val path = "/dispatchkubernetes/api/build/job/expose"
        val requestBody = RequestBody.create(JSON_CONTENT_TYPE, JsonUtil.toJson(createServiceReq))

        val request = buildPost(path, requestBody, mutableMapOf("X-DEVOPS-UID" to getUserId()))
        val responseContent = request(request, "kubernetes service失败")
        logger.debug("create kubernetes service response: $responseContent")

        return JsonUtil.fromJson(responseContent, object : TypeReference<Result<DispatchTaskResp?>>() {})
    }

    fun getJobStatus(jobName: String): Result<DispatchBuildStatusResp> {
        val path = "/dispatchkubernetes/api/build/job/" + jobName + "/status"
    ...
    fun getJobLogs(jobName: String, sinceTime: Int): Result<DispatchJobLogResp> {
        val path = "/dispatchkubernetes/api/build/job/" + jobName + "/logs?sinceTime=" + sinceTime
        val request = buildGet(path, mutableMapOf("X-DEVOPS-UID" to getUserId()))
    ...
    fun getTask(taskId: String): Result<DispatchBuildStatusResp> {
        val path = "/dispatchkubernetes/api/build/task/status?taskId=" + taskId
        val request = buildGet(path, mutableMapOf("X-DEVOPS-UID" to getUserId()))
    ...
    fun dockerBuildAndPush(dispatchBuildImageReq: DispatchBuildImageReq): Result<DispatchTaskResp?> {
        val path = "/dispatchkubernetes/api/build/image/buildPush"
